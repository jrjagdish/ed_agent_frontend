...
const handelApikeyGeneration = async () => {
  try {
    ...
    if (response.ok) {
      console.log("Success:", data.api_key);
      return data;
    } else {
      ...
      <aside className="hidden lg:flex sticky top-0 h-screen w-72 flex-col p-5 z-20">
        <div className="flex-1 bg-white/[0.02] backdrop-blur-3xl border border-white/5 rounded-[24px] p-5 flex flex-col gap-6 shadow-2xl">
          <div className="flex items-center gap-3 p-3 rounded-xl bg-gradient-to-r from-white/5 to-transparent border border-white/5">
            <div className="size-9 rounded-full bg-gradient-to-tr from-indigo-500 to-cyan-400 p-[1px]">
              <img
                ...
                className="rounded-full bg-[#03050c]"
                alt="avatar"
              />
            </div>
          ...
          <div className="mt-auto p-4 rounded-[16px] bg-indigo-500/5 border border-indigo-500/10 relative overflow-hidden group">
            <p className="text-[9px] font-bold text-indigo-300 uppercase mb-2">
              Usage
            </p>
          ...
          <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 py-2">
            <div>
              ...
              <p className="text-slate-500 text-xs font-medium">
                v2.0.4 • All systems nominal
              </p>
            </div>
            <motion.button
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              onClick={handelApikeyGeneration}
              className="flex items-center gap-2 bg-white text-black px-5 py-2.5 rounded-[12px] font-bold text-xs hover:bg-slate-200 transition-colors"
            >
            ...
            <div className="flex justify-between items-center mb-8">
              ...
              <div className="flex bg-[#03050c] p-1 rounded-lg border border-white/5">
                {["1H", "24H", "7D"].map((t) => (
                  <button
                    key={t}
                    className={`px-4 py-1.5 text-[10px] font-black rounded-md transition-all ${t === "24H" ? "bg-indigo-600 text-white" : "text-slate-500 hover:text-white"}`}
                  >
                    {t}
                  </button>
                ))}
...
const NavItem = ({ icon, label, active = false }) => (
  <button
    className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all font-bold text-xs ${active ? "bg-white text-black shadow-lg" : "text-slate-500 hover:bg-white/5 hover:text-white"}`}
  >
...
const DashboardRow = ({ name, status, time }) => (
  <tr className="hover:bg-white/[0.01] transition-colors group">
    ...
    <td className="px-6 py-4">
      <div
        className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[9px] font-black uppercase border ${status === "Healthy" ? "bg-emerald-500/5 text-emerald-400 border-emerald-500/10" : "bg-orange-500/5 text-orange-400 border-orange-500/10"}`}
      >
        <div
          className={`size-1 rounded-full ${status === "Healthy" ? "bg-emerald-400 shadow-[0_0_8px_#34d399]" : "bg-orange-400"}`}
        />
        {status}
      </div>
    </td>
